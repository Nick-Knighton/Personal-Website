...
const DropdownMenu = ({ options, setSelectedOptions, dropdownTitle }) => {
  ...
  function deselectAllOptions() {
    const optionCheckboxes = document.querySelectorAll(`#${dropdownTitle} input[type="checkbox"]`);
  ...
  function selectAllOptions() {
    ...
    const allOptions = Array.from(optionCheckboxes).map((checkbox) => checkbox.value);
  ...
  return (
    <div>
        ...
        <div id={dropdownTitle} className="z-10  hidden  w-56 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
            <h6 className="mb-3 text-sm font-medium text-gray-900 dark:text-white">{dropdownTitle}:</h6> 
            ...
            <ul className="space-y-2 text-sm" aria-labelledby={`dropdown${dropdownTitle}`}>
                ...
                <li className="flex items-center" key={item.value}>
                    ...
                    <label htmlFor={item.value} className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                    {item}
                    </label>
                </li>
                ))}
            ...
            <div className="flex justify-center mt-4">
                ...
                <button className="ml-4 text-sm text-black hover:text-gray-700" 
